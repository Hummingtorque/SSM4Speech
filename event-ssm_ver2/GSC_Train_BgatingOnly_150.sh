...
#SBATCH --job-name=gsc_train_bgating_only
...
CMD=(python -u run_training.py data_dir="${GSC_DIR}" \
  ...
  training.num_epochs=150 training.validate_on_test=false \
  ...
  +model.ssm.input_gate=true +model.ssm.input_gate_rank=0 +model.ssm.input_gate_mode=sigmoid +model.ssm.input_gate_min=0.0 \
