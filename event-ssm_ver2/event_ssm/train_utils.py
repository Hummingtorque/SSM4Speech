...
def training_step(
    ...
    def loss_fn(params):
        ...
        # Sanitize logits to avoid NaNs/Infs propagating into the loss
    ...
    # Sanitize gradients to prevent NaNs/Infs from corrupting optimizer state
    grads = jax.tree_util.tree_map(lambda g: jnp.nan_to_num(g, nan=0.0, posinf=0.0, neginf=0.0), grads)

    train_state = train_state.apply_gradients(grads=grads)

    # Sanitize updated parameters (in case optimizer state had non-finite values)
    clean_params = jax.tree_util.tree_map(lambda p: jnp.nan_to_num(p, nan=0.0, posinf=0.0, neginf=0.0), train_state.params)
    train_state = train_state.replace(params=clean_params)
...
def get_optimizer(opt_config):
    ...
    ssm_lrs = ["B", "Lambda", "Lambda_re", "Lambda_im"]
